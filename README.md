...
- **데스크톱 팻** — 모든 웹페이지 위에 오버레이로 떠서 중력으로 떨어지고 바닥을 뛰어다닌다. 클릭은 막지 않는다(`pointer-events:none`).
- **요소에 올라타기** — 화면에 보이는 버튼·링크·이미지 등(40~400px)으로 걸어가 그 위에 올라앉고, 요소가 스크롤로 사라지거나 제거되면 떨어진다.
- **상태 표정** — 걸을 땐 걷기 애니메이션, 멈췄을 때 배고프면(≥70) 배고픈 얼굴, 행복하면(≥90) 활짝 웃는 얼굴.
...
- 크롬 툴바 오른쪽의 **사이드 패널 아이콘**을 누르고 목록에서 "크롬 팻"을 선택한다.
- 패널에 팻의 배고픔·행복 수치와 **먹이 주기** 버튼이 보인다.
- (참고: 확장 아이콘 클릭으로 패널이 자동으로 열리진 않는다. 크롬의 사이드 패널 선택기로 연다.)
...
스프라이트 시트(`src/assets/pet.png`)는 6프레임(idle·walk1·walk2·fall·happy·hungry)이며, 각 64×104px다. 원본 프레임 이미지에서 다시 만들려면.
