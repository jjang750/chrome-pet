...
function startPetOverlay(): void {
  ...
  // 낮잠(sleeping)은 연출 전용이다. 애니메이션 주기(약 38.6초)마다 mood 를 회복시키면
  // decay(배고픔 +10/시간)를 186배 앞질러 배고픔이 영구히 0 에 고정된다 → 상태 보상을 붙이지 않는다.
  // 배고픔·행복도는 시간 경과(decay)와 사용자 행동(feed)으로만 변한다.
