...
다음 세션(사람/에이전트)이 이어받기 위한 인수인계 문서. 작성 2026-07-23.

## 1. 현재 상태 (한 줄)

MV3 크롬 확장. 웹페이지 위 데스크톱 팻(오버레이) + 사이드패널 상태 UI. `npm run check` 그린(80 tests), `origin/main`과 완전 동기화(HEAD `fad2de9`). GitHub: `jjang750/chrome-pet`.
...
- **배고픔 알람** — `chrome.alarms` 1분 주기로 decay(시간 경과 시 배고픔↑·행복↓).
- **추가 애니메이션** — 주기적 낮잠(sleep), 행복 낮으면 칭얼(want_play), 행복 높으면 happy.
...
├─ chrome/storage.ts # chrome.storage 어댑터 (loadPet/savePet)
├─ background/index.ts# service worker (알람 생성·decay, setPanelBehavior, 전역 에러 → __errors)
├─ content/index.ts   # 오버레이 rAF 렌더 + 요소 타깃팅 + 드래그·먹이·놀이·게이지 트리거 + 알림 프록시
└─ sidepanel/         # 상태 게이지 UI + 먹이 버튼
scripts/  build.mjs · validate-manifest.mjs · make-sprite.mjs
tests/e2e/ smoke·feed·alarm·overlay·perch·perch-hop·drag·eat.spec.ts
```
...
| 명령 | 역할 |
|---|---|
| `npm run check` | lint→typecheck→test(vitest 80)→validate→build. **완료 게이트, 이거 통과가 곧 완료** |
...
- **주의**: 세션 중 무언가(훅 추정)가 커밋을 origin/main에 **자동 push**하는 동작이 관찰됨. 그래서 로컬 커밋이 곧 origin에 올라감. 직접 `git push`는 안전 게이트에 막혀서, 필요 시 사용자가 `! git push origin main` 실행.
...
- **의사결정·근거**는 `context-notes.md`(결정 1~9)에, 체크리스트는 `checklist.md`에 있음.
