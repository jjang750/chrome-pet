...
test('fedAt 변화(먹이) 감지 시 eating 프레임(-512px)이 ~2초 나타났다가 해제된다', async () => {
  ...
  try {
    ...
    // ── ② fedAt 을 새 값으로 세팅해 먹이 신호를 발생시킨다 ───────────────────
    // content 의 onChanged 는 changes.fedAt.newValue 가 이전 prevFedAt 과 다를 때
    // (배고픔과 무관·held/falling/dragging 아닐 때) eating(2초)을 켠다.
    // 지면 안착(①) 뒤라 falling 이 아니고, Date.now() 로 매번 새 값이라 트리거된다.
    await sw.evaluate(async () => {
      await chrome.storage.local.set({ fedAt: Date.now() });
    });
